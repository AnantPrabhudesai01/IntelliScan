...
export default function SignalsCard() {
  ...
  return (
    <div className="bg-white dark:bg-[#161c28] border border-gray-100 dark:border-gray-800 rounded-[2rem] p-8 relative overflow-hidden group shadow-xl shadow-brand-600/5">
      {/* Dynamic Background Blob */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-brand-500/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl group-hover:scale-125 transition-transform duration-1000"></div>
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        ...
        <button onClick={fetchSignals} className="p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl transition-colors text-brand-500 font-bold text-xs uppercase tracking-tighter">
           Sync AI
        </button>
      </div>
          ...
          signals.map((sig) => (
            <div key={sig.id} className="flex items-start gap-4 p-4 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-all border border-transparent hover:border-gray-100 dark:hover:border-gray-800 group/sig">
              <div className={`p-2.5 rounded-xl border border-white dark:border-gray-800 shadow-sm ${
                ...
                sig.type === 'career' ? 'bg-brand-100 text-brand-600 dark:bg-brand-900/30 dark:text-brand-400' :
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                   <h4 className="font-bold text-gray-900 dark:text-white text-sm tracking-tight group-hover/sig:text-brand-500 transition-colors uppercase">{sig.title}</h4>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link to={`/dashboard/contacts?id=${sig.id.split('-')[1]}`} className="text-[10px] font-bold text-brand-500 hover:underline flex items-center gap-0.5 opacity-0 group-hover/sig:opacity-100 transition-opacity">
      ...
      <Link to="/dashboard/contacts" className="mt-8 block w-full text-center py-3 bg-gray-50 dark:bg-gray-800/50 rounded-2xl text-xs font-black text-brand-600 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-brand-900/10 transition-all uppercase tracking-widest border border-gray-100 dark:border-gray-800">
