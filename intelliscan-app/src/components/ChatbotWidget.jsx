    ...
    <div className="fixed bottom-6 right-6 z-[9999] font-body text-left">
      {isOpen ? (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl w-80 sm:w-96 flex flex-col h-[500px] overflow-hidden animate-in slide-in-from-bottom flex-shrink-0">
          {/* Header */}
          <div className="bg-gradient-to-r from-brand-600 to-brand-700 p-4 flex items-center justify-between shadow-sm z-10">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-950/50 relative style-scrollbar">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${m.role === 'user' ? 'bg-brand-100 dark:bg-brand-900/50 text-brand-600 dark:text-brand-400' : 'bg-brand-600 text-white shadow-sm'}`}>
          ...
          <div className="p-3 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full pl-4 pr-12 py-3 bg-gray-100 dark:bg-gray-800/50 border border-transparent focus:border-brand-500/50 focus:bg-white dark:focus:bg-gray-800 rounded-xl text-sm outline-none text-gray-900 dark:text-white transition-all shadow-inner"
              />
              <button 
                onClick={handleSend}
                disabled={!input.trim() || loading}
                className="absolute right-2 p-1.5 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-lg transition-colors shadow-sm"
              >
        ...
        <button 
          ...
          className="bg-brand-600 hover:bg-brand-700 text-white p-4 rounded-full shadow-2xl hover:shadow-brand-500/30 transition-all hover:scale-105 group relative"
        >
