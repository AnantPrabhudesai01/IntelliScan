    ...
    <div className="space-y-3">
      <div className="relative">
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold focus:ring-2 focus:ring-brand-500 outline-none placeholder:text-gray-400 group-hover:border-brand-400/50"
        />
        <Mail className="absolute left-3.5 top-3 text-gray-400" size={16} />
        <button
          type="button"
          onClick={addAttendee}
          className="absolute right-2 top-1.5 p-1.5 bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-colors shadow-sm"
        >
          <UserPlus size={16} />
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {attendees.map((att) => (
          ...
          >
            <div className="w-5 h-5 rounded-full bg-brand-100 dark:bg-brand-900/40 text-brand-600 dark:text-brand-400 flex items-center justify-center text-[10px]">
