...
export default function BookingPage() {
  ...
  if (!data?.booking_link) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0f] text-white p-6 text-center">
      ...
      <Link to="/" className="px-8 py-3 bg-indigo-600 rounded-2xl font-black uppercase text-sm">Back to Home</Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-[#dde2f3] flex items-center justify-center p-4 selection:bg-indigo-600">
      <div className="w-full max-w-5xl bg-[#0e131f] border border-gray-800 rounded-[3rem] shadow-2xl flex flex-col md:flex-row overflow-hidden min-h-[600px] animate-in zoom-in-95 duration-500">
        ...
        <div className="md:w-1/3 border-b md:border-b-0 md:border-r border-gray-800 p-10 bg-gradient-to-br from-[#0e131f] to-[#151c2c]">
          <div className="w-16 h-16 rounded-3xl bg-indigo-600 flex items-center justify-center mb-8 shadow-xl shadow-indigo-500/20">
                      ...
                      return (
                        <button
                          ...
                          className={`h-12 w-12 rounded-2xl flex items-center justify-center text-sm font-black transition-all ${
                            isSelected ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20 scale-110' : 'hover:bg-gray-800 text-gray-400'
                          }`}
                        >
                          {format(day, 'd')}
                        </button>
                  ...
                  <div className="space-y-3">
                    {['09:00', '10:00', '11:30', '13:00', '14:30', '16:00'].map(time => (
                      <button
                        ...
                        className="w-full py-4 text-center bg-gray-900 border border-gray-800 rounded-2xl text-sm font-black hover:border-indigo-500 hover:text-indigo-400 transition-all active:scale-95 group"
                      >
                        {time}
                      </button>
                    ))}
          ...
          {step === 'details' && (
            <div className="h-full flex flex-col animate-in slide-in-from-right duration-300">
              <button 
                ...
                className="flex items-center gap-2 text-xs font-black text-indigo-500 uppercase tracking-widest mb-8 hover:text-indigo-400 transition-colors"
              >
                <ChevronLeft size={16} /> Back to Selection
              </button>
              <h2 className="text-2xl font-black text-white mb-2">Confirm Details</h2>
              <p className="text-sm font-bold text-indigo-500 mb-8 uppercase tracking-widest">
              ...
              <form onSubmit={handleBook} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-6 py-4 bg-[#0a0a0f] border border-gray-800 rounded-2xl text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest flex items-center gap-2">
                    <MessageSquare size={12} className="text-indigo-600" /> Meeting Notes
                  </label>
                  <textarea
                    value={form.notes}
                    onChange={(e) => setForm({ ...form, notes: e.target.value })}
                    className="w-full px-6 py-4 bg-[#0a0a0f] border border-gray-800 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none h-32 resize-none"
                ...
                <button
                  ...
                  className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black text-sm uppercase tracking-widest shadow-2xl shadow-indigo-500/20 active:scale-95 transition-all disabled:opacity-50"
                >
          ...
          {step === 'success' && (
            <div className="h-full flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-500">
               ...
               <div className="p-6 bg-gray-900 border border-gray-800 rounded-3xl w-full max-w-sm text-center">
                 ...
                 <p className="text- indigo-500 font-bold">{selectedSlot} ({Intl.DateTimeFormat().resolvedOptions().timeZone})</p>
               </div>
            </div>
          )}
        </div>
