...
function EndpointDetails({ method, path, desc, reqBody, resBody }) {
  const methodColors = {
    GET: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/20',
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0e131f] text-gray-900 dark:text-[#dde2f3] font-body flex flex-col lg:flex-row">
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden fixed">
        <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-indigo-600/5 rounded-full blur-[120px] translate-x-1/3 -translate-y-1/3"></div>
      </div>
          ...
          {DOC_SECTIONS.map(sec => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
                ${activeHash === sec.id 
                  ? 'bg-indigo-50 dark:bg-indigo-600/10 text-indigo-700 dark:text-indigo-400' 
                  : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white'}`}
            >
              <sec.icon size={16} className={activeHash === sec.id ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'} />
              {sec.title}
            </button>
          ))}
        </div>
        <div className="p-6 border-t border-gray-200 dark:border-white/10 shrink-0">
          <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-500/20 rounded-xl p-4">
            <p className="text-[10px] font-bold text-indigo-800 dark:text-indigo-300 uppercase tracking-widest mb-1">Current Version</p>
            <p className="text-sm font-mono font-bold text-indigo-900 dark:text-indigo-400">v2.1.4 (Stable)</p>
          </div>
        </div>
      </aside>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <section id="auth" className="scroll-mt-24 mb-24">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6 leading-relaxed">
              The IntelliScan API uses Bearer Tokens for authentication. You can generate active API keys from the <Link to="/marketplace" className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline">Integrations & API Settings</Link> dashboard.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              <div className="p-6 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl">
                ...
                <ul className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <li className="flex justify-between items-center"><span className="font-semibold text-primary dark:text-indigo-400">Enterprise</span> <span className="font-mono bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300 px-2 py-0.5 rounded border border-indigo-200 dark:border-indigo-500/20">Custom Limit</span></li>
                </ul>
              </div>
